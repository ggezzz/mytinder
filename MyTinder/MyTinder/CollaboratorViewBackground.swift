...
class CollaboratorViewBackground: UIView, DraggableViewDelegate {
    ...
    func createDraggableViewWithData(at index: Int) -> DraggableView{
        ...
        if let profileImageLink = fren.profilePhotoURL  {
            let profileImageUrl = NSURL(string: profileImageLink)
            draggableView.profilePhoto.sd_setImage(with: profileImageUrl as URL!)
        } else {
            draggableView.profilePhoto.image = UIImage(named: "avatar-2")
        }
    ...
    func cardSwipedLeft(_ card: UIView) {
        ...
        self.response(responseType: "reject", friendID: frenView.castID)
    ...
    func response(responseType:String, friendID:String){
        ...
        if responseType == "accept" {
            ...
            DataService.userRef.child(Session.currentUserUid).child("friends").child(friendID).removeValue()
        }
